...
class UpdateError(DatabaseError):
    ...
    def __init__(self, player_id: str, player_type: str, message: Optional[str] = None):
        self.player_id = player_id
        self.player_type = player_type
        self.message = message or f"Failed to update {player_type} player with ID '{player_id}'."
        super().__init__(self.message)
...
class DeleteError(DatabaseError):
    ...
    def __init__(self, player_id: str, player_type: str, message: Optional[str] = None):
        self.player_id = player_id
        self.player_type = player_type
        self.message = message or f"Failed to delete {player_type} player with ID '{player_id}'."
        super().__init__(self.message)


class SearchError(DatabaseError):
    """Raised when a search operation fails or returns no results."""
    def __init__(self, search_field: str, search_value: str, message: Optional[str] = None):
        self.search_field = search_field
        self.search_value = search_value
        self.message = message or f"No players found with {search_field} '{search_value}'."
        super().__init__(self.message)
